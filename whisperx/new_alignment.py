...
def align(
    ...
    for sdx, segment in enumerate(transcript):
        ...
        waveform_segment, emissions = audio_model_inference(model, device, model_type, waveform_segment)
    ...
    with torch.inference_mode():
        for start_idx in tqdm(range(0, total_frames, chunk_size)):
            ...
            if model_type == "torchaudio":
                emissions, _ = model(chunk.to(device))
            elif model_type == "huggingface":
                emissions = model(chunk.to(device)).logits
            else:
                raise NotImplementedError(f"Align model of type {model_type} not supported.")
            # Apply log softmax to emissions
            emissions = torch.log_softmax(emissions, dim=-1)  # [channels, num_tokens, vocab_size]
            emissions_list.append(emissions)

    # Concatenate all emissions from chunks
    emissions = torch.cat(emissions_list, dim=1)

    return waveform_segment, emissions
